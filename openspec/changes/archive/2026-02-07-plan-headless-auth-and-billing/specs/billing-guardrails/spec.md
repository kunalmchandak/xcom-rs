...
`xcom-rs` は `billing estimate` で、対象操作の想定コストを実行前に返さなければならない（MUST）。

#### Scenario: 投稿操作の見積取得
- **Given** 利用者が `billing estimate tweets.create --text "hello" --output json` を実行する
- **When** 見積結果が返る
- **Then** `cost.credits` と `cost.usdEstimated` が返る
